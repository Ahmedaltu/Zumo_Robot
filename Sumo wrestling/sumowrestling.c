...
void zmain(void){
    ...
    while(dig.L1 == 1 && dig.R1 == 1)           //when robot on line outside the ring
    {
    ...
    IR_wait();                                  // robot waits IR button on the ring to enter it
    motor_forward(245, 300);
    start = xTaskGetTickCount();                // start time counting 
    ...
    {   
       ...
       if(dig.L3==1 || dig.L2 ==1 || dig.L1==1) 
        ...
        if(SW1_Read() == 0) {                         //user button pressed, robot inside the ring
            end = xTaskGetTickCount();                 //end time counting
