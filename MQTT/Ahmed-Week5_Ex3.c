...
void zmain(void)
{
    ...
    MotorReflect();  // starts reflectance, IR, motor and to go forward. 
    ...
    while(count < 2){
        ...
        if (touching == 0 && dig.L3 == 1 && dig.L2 == 1 && dig.L1 == 1 && dig.R1 == 1 && dig.R2 == 1 && dig.R3 == 1) {
            ...
            if (count == 1) {
                motor_forward(0, 0);
                print_mqtt(LAP_TOPIC,"\nWaiting for IR signal\n");
                IR_wait();
                time = xTaskGetTickCount();
                print_mqtt(LAP_TOPIC,"Timer starts");
            }
            if(count == 2){
             print_mqtt(LAP_TOPIC,"\nTime elapsed: %d s", time/1000);
             motor_forward(0,0);
            }
